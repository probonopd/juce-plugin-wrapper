...
public:
  ...
  bool canAddBus(bool inputBus) const override {
    return plugin->canAddBus(inputBus);
  }
  bool canRemoveBus(bool inputBus) const override {
    return plugin->canRemoveBus(inputBus);
  }
  bool supportsDoublePrecisionProcessing() const override {
    return plugin->supportsDoublePrecisionProcessing();
  }
  ...
  void setCurrentProgramStateInformation(const void *data, int bytes) override {
    plugin->setCurrentProgramStateInformation(data, bytes);
  }
  void getCurrentProgramStateInformation(juce::MemoryBlock &destData) override {
    plugin->getCurrentProgramStateInformation(destData);
  }
  // Instead I provide these functions, so state get/set can be controlled from
  // GUI
...
protected:
  ...
  bool canApplyBusesLayout(const BusesLayout &layouts) const override {
    ...
    return plugin->setBusesLayout(layouts);
  }
  bool canApplyBusCountChange(bool isInput, bool isAddingBuses,
                              BusProperties &outNewBusProperties) override;

  static BusesProperties
  getBusesPropertiesFromProcessor(AudioProcessor *processor);
